...
class TTSInterface(metaclass=abc.ABCMeta):
    ...
    @abc.abstractmethod
    def generate_audio(self, text: str, file_name_no_ext=None, speed: float = 1.0) -> str:
        """
    ...
    def generate_cache_file_name(self, file_name_no_ext=None, file_extension="wav"):
        ...
        if file_name_no_ext is None:
            ...
            # 生成随机哈希值（这里用7位随机16进制数模拟）
            random_bytes = random.randbytes(4)  # 生成4字节随机数据
            hash_str = hashlib.sha256(random_bytes).hexdigest()[:7]  # 取前7位
            
            # 拼接文件名
            file_name_no_ext = f"{time_str}-{hash_str}"
